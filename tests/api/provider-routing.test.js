...
test('falls back to a second full diagnosis provider when primary diagnosis fails', async () => {
  let fallbackInput;
  const req = { method:'POST', body:{ diagnosis } };
  const res = mockRes();
  await handleDiagnosisRequest(req, res, {
    primaryProvider: { name:'deepseek', diagnose: async () => { throw new Error('primary unavailable'); } },
    fallbackProvider: { name:'openai', diagnose: async (value) => { fallbackInput = value; return { mode:'question', question:{ key:'cost', question:'最近成本变化多少？', reason:'OpenAI 接管完整问诊' }, findings:[] }; } },
    reviewerProvider: null
  });
  assert.equal(res.statusCode, 200);
  assert.equal(res.body.mode, 'question');
  assert.equal(res.body.question.reason, 'OpenAI 接管完整问诊');
...
test('reviewer failure does not discard a valid primary diagnosis', async () => {
  ...
  await handleDiagnosisRequest(req, res, {
    ...
    reviewerProvider: { name:'openai', review: async () => { throw new Error('review unavailable'); } }
  });
  assert.equal(res.statusCode, 200);
