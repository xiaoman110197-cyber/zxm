...
export function auditWorkbook(workbook) {
  if (!workbook || !Array.isArray(workbook.sheets)) {
    throw new TypeError('parsed workbook is required');
  }
    ...
    sheet.rows.forEach((row, index) => {
      ...
      for (const field of sheet.headers) {
        ...
        if (value === null || value === undefined || value === '') {
          errors.push({ type: 'missing_value', sheet: sheet.name, row: rowNumber, field });
        }
      }
    });
  }
  ...
  if (orders?.headers.includes('营业额')) {
    metrics.revenue = numericSum(orders.rows, '营业额');
  }

  const costs = findSheet(workbook, ['成本']);
  if (costs?.headers.includes('成本')) {
    metrics.cost = numericSum(costs.rows, '成本');
  }
  ...
  if (orders && summary && Number.isFinite(metrics.revenue)) {
    ...
    if (typeof summaryRevenue === 'number' && Number.isFinite(summaryRevenue) && summaryRevenue !== metrics.revenue) {
      errors.push({
        type: 'cross_sheet_total_mismatch',
        metric: '营业额',
        sourceSheet: orders.name,
        summarySheet: summary.name,
        expected: metrics.revenue,
        actual: summaryRevenue
      });
    }
  }
